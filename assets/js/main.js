...
function multiple_selected(options){
    ...
    $(config.role_select_all).on("click", function() {
        $(config.role_select).prop('checked', this.checked);
        $(config.role_select).change();
    });

    $(config.role_select).on("change", function() {
    	let total_selected = $(config.role_select+":checked").length;
    	let total_role_select = $(config.role_select).length;


        $(config.role_select_all).prop('checked', (total_selected >= total_role_select ? true : false));
                ...
                [].slice.call(table.find('th.sort-btn')).forEach(function(th_sort) {
                    ...
                    th.on('click', function() {
                        ...
                        tbody.find('td').filter(function(){
                            ...
                            return a.replace('.', '').localeCompare(b.replace('.', ''), undefined, {numeric: true, sensitivity: 'base'});
                        }, function(){
