        ...
        build_title: function(select_container, default_select, title) {
            ...
            if(isMultiple) {
                ...
                default_select.find('option[selected]').each((index, option) => {
                    ...
                    ul.append('<li class="' + this._class.select_title_multiple_li + '"><span ' + this.set_role('remove_selected') + this._data.id_option + '="' + option.getAttribute(this._data.id_option) + '">' + this._icon.remove + '</span><span>' + (html_option || option.innerText || option.value) + '</span></li>');
                });
            ...
            if(default_select.find('option').length > 0) {
                ...
                default_select.find('option').each((index, option) => {
                    ...
                    if(value.trim() == '') {
                        ...
                        html += '<li class="' + this._class.option_li + ($(option).attr('selected') || (!isMultiple && $(option).is(':selected')) ? ' selected' : '') + '" ' + this._data.id_option + '="' + id_option + '">' + (html_option || value) + '</li>';
                    }
                });
                html += '</ul>';
            } else {
