        ...
        upload_image.change(function(event) {
            ...
            if (files && files.length > 0) {
                ...
                reader.onload = function(event) {
                    ...
                    $.dialogShow({
                        title: config.title,
                        content: '<img class="cropper-responsive" id="crop_image" src="'+reader.result+'">',
                        desc: '\
                        ...
                        onConfirm: function() {
                            ...
                            cropper.getCroppedCanvas({width: config.width, height: config.height}).toBlob(function(blob) {
                                url = URL.createObjectURL(blob);
                                var reader = new FileReader();
                                reader.readAsDataURL(blob);
                                reader.onloadend = function() {
                                    if(config.type === "avatar"){
                                        $(config.preview).attr('src', reader.result);
                                        $(config.preview).show();
                                        btn_save.val("Save Avatar");
                                    }
                                    if(config.type === "cover"){
                                        $(config.preview).css('background-image', 'url('+reader.result+')');
                                        btn_save.val("Save Cover");
                                    }
