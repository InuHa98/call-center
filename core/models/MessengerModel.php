...
class Messenger extends Model {
	...
	protected static $table = 'core_messenger';
	...
	public static function count_new_inbox()
	{
		return parent::count([
			...
			'last_message.to_user_id' => Auth::$id,
			'AND' => [
				'{table}.is_spam_to[!]' => Auth::$id,
				'{table}.is_spam_from[!]' => Auth::$id
			]
		], "SELECT count(*) FROM <{table}> JOIN <{table}> AS <last_message> ON <last_message.id> = (SELECT <last_id.id> FROM <{table}> AS <last_id> WHERE <last_id.refid> = <{table}.id> ORDER BY <time> DESC LIMIT 1)");
	}
	...
	public static function list_inbox($is_spam = false, $type = null, $keyword = null, $page = null)
	{
		...
		SELECT 
			...
			<core_user_from.is_ban> AS <user_from_user_ban>,
			...
			<core_user_to.is_ban> AS <user_to_user_ban>,
