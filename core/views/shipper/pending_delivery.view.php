...
<div class="box">
    ...
    <div class="box__header"><i class="fas fa-truck-loading text-warning"></i><span class="padding-x-2"><?=lang('system', 'pending_delivery');?></span></div>
    <div class="box__body">
                ...
                <div class="form-control">
                    ...
                    <?php foreach($list_product as $product):
                        ...
                        <div class="user-infomation">
                            ...
                            <span class="user-display-name">'.$product['name'].' ('.Currency::format($product['price']).' '.$product['currency'].')  - '.$product['stock'].' '.lang('label', 'stock').'</span>
                        </div>';
                    ?>
                        ...
                        <td>
                            <div class="drop-menu">
                                ...
                                <ul class="drop-menu__content" data-id="<?=$order['id'];?>">
                                    <li role="mark-delivering" class="text-primary">
                                        <i class="fas fa-shipping-fast"></i> <?=lang('button', 'mark_delivering');?>
                                    </li>
                                    <li role="cancel">
                                        <i class="fas fa-times"></i> <?=lang('button', 'mark_cancel');?>
                                    </li>
                                </ul>
                            </div>
                        </td>
        ...
        <div class="margin-y-2">
            <span class="drop-menu disabled" role="action-order">
                ...
                <ul class="drop-menu__content">
                    <li role="export-excel" class="border-bottom">
                        <i class="fas fa-file-excel"></i> <?=lang('button', 'export_excel');?> <span role="multiple_selected_count">(0)</span>
                    </li>
                    <li role="mark-delivering-order" class="text-primary">
                        <i class="fas fa-shipping-fast"></i> <?=lang('button', 'mark_delivering');?> <span role="multiple_selected_count">(0)</span>
                    </li>
                    <li role="cancel-order">
                        <i class="fas fa-times"></i> <?=lang('button', 'mark_cancel');?> <span role="multiple_selected_count">(0)</span>
                    </li>
                </ul>
            </span>
        </div>
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        role_click('mark-delivering', function(self) {
            const form = $('\
            <form method="POST">\
                <?=Security::insertHiddenToken();?>\
                <input type="hidden" name="<?=orderController::INPUT_ACTION;?>" value="<?=orderController::ACTION_MARK_DELIVERING;?>" />\
        ...
        role_click('cancel', function(self) {
            const form = $('\
            <form method="POST">\
                <?=Security::insertHiddenToken();?>\
                <input type="hidden" name="<?=orderController::INPUT_ACTION;?>" value="<?=orderController::ACTION_CANCEL_ORDER;?>" />\
                <input type="hidden" name="<?=orderController::INPUT_ID;?>" value="' + self.parent().data('id') + '" />\
            </form>');
            $('body').append(form);
			form.submit();
        ...
        role_click('mark-delivering-order', async function(self) {
			if(await comfirm_dialog('<?=lang('button', 'mark_delivering');?>', '<?=lang('system', 'txt_can_undo');?>') !== true)
			{
				return;
			}
			request_action('<?=orderController::ACTION_MARK_DELIVERING;?>');
        });

        role_click('cancel-order', async function(self) {
			if(await comfirm_dialog('<?=lang('button', 'mark_cancel');?>', '<?=lang('system', 'txt_can_undo');?>') !== true)
			{
				return;
			}
			request_action('<?=orderController::ACTION_CANCEL_ORDER;?>');
        });

        role_click('export-excel', function(self) {
            request_action('<?=orderController::ACTION_EXPORT_EXCEL;?>');
        });
