...
$id_order = isset($order) ? ' - #'.$order['id'] : null;
?>
...
<div class="box">
	<form id="form-validate" method="POST">
        ...
        <div class="box__header"><i class="far fa-phone-volume text-primary"></i><span class="padding-x-2"><?=lang('order', 'txt_calling');?> <?=$id_order;?></span></div>
		<div class="box__body">
            <?php if($id_order): ?>
            ...
            <div class="card-product">
                <img class="card-product__image" src="<?=Product::get_image(['image' => $order['product_image']]);?>" />
                <div class="card-info-order">
                    ...
                    <div class="label"><?=lang('label', 'description');?>:</div>
                    <ul class="card-product__info">
                    <?php if($product): 
                        $product_desc = json_decode($product['desc'], true);
                        if(!is_array($product_desc)) {
                            $product_desc = [$product_desc];
                        }
                    ?>
                    <?php foreach($product_desc as $desc): ?>
                        <li><?=_echo($desc);?></li>
                    <?php endforeach; ?>
                    <?php endif; ?>
                    </ul>
                    ...
                    <?php if($order['call_at']): ?>
                        <div class="card-product__last-call">
                            <span class="label"><?=lang('label', 'last_call');?>:</span> <?=_time($order['call_at']);?>
                        </div>
                    <?php endif; ?>            
                </div>
            ...
            <?php else: ?>
                <div><?=lang('errors', 'empty_order');?></div>
            <?php endif; ?>
		</div>
        <?php if($id_order): ?>
