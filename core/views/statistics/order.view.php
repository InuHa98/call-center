...
<div class="box margin-t-4">
    <div class="box__header"><i class="far fa-chart-bar"></i> <span class="padding-x-2"><?=lang('system', 'order_statistics');?></span></div>
    <div class="box__body">
            ...
            <div class="form-control">
                ...
                <?php foreach($list_product as $product):
                    ...
                    <div class="user-infomation">
                        ...
                        <span class="user-display-name">#'.$product['id'].' - '.$product['name'].' ('.Currency::format($product['price']).' '.$product['currency'].') - '.$product['stock'].' '.lang('label', 'stock').'</span>
                    </div>';
                ?>
            ...
            <table class="table-statistics table-sort">
                <thead>
                    <tr>
                        ...
                        <th></th>
                        <th class="sort-btn align-center tooltip" title="<?=lang('statistic', 'epo');?>">EPO (<?=$currency;?>)</th>
                        <th class="sort-btn align-center tooltip" title="<?=lang('statistic', 'estimated_profit');?>">Estimated Profit (<?=$currency;?>)</th>
                        <th class="sort-btn align-center tooltip" title="<?=lang('statistic', 'profit');?>">Profit (<?=$currency;?>)</th>
                ...
                foreach($list_statistics as $statistics):
                    ...
                    $epo_rate = calculate_epo($statistics);
                    $total_estimated_profit += $statistics['estimated_profit'];
                    $total_profit += $statistics['profit'];
                    $total_epo += $epo_rate;
                ...
                ?>
                    <tr>
                        ...
                        <td class="nowrap align-center"><?=render_count($epo_rate);?> <?=($epo_rate ? '<span class="time">('.Currency::format(round($product['to_default_currency'] * $epo_rate)).' '.Currency::DEFAULT_CURRENCY.')</span>' : null);?></td>
                        <td class="nowrap align-center"><?=render_count($statistics['estimated_profit']);?> <?=($statistics['estimated_profit'] ? '<span class="time">('.Currency::format(round($product['to_default_currency'] * $statistics['estimated_profit'])).' '.Currency::DEFAULT_CURRENCY.')</span>' : null);?></td>
                        <td class="nowrap align-center"><?=render_count($statistics['profit'], 'text-primary');?> <?=($statistics['profit'] ? '<span class="time">('.Currency::format(round($product['to_default_currency'] * $statistics['profit'])).' '.Currency::DEFAULT_CURRENCY.')</span>' : null);?></td>
                ...
                <tfoot>
                    <tr>
                        <?php
                            $total_epo_rate = $total_order > 0 ? round($total_epo / $total_order) : 0;
                        ?>
                        ...
                        <th><?=render_count($total_epo_rate);?>  <?=($total_epo_rate ? '<span class="time">('.Currency::format(round($product['to_default_currency'] * $total_epo_rate)).' '.Currency::DEFAULT_CURRENCY.')</span>' : null);?></th>
                        <th><?=render_count($total_estimated_profit);?>  <?=($total_estimated_profit ? '<span class="time">('.Currency::format(round($product['to_default_currency'] * $total_estimated_profit)).' '.Currency::DEFAULT_CURRENCY.')</span>' : null);?></th>
                        <th><?=render_count($total_profit, 'text-primary');?>  <?=($total_profit ? '<span class="time">('.Currency::format(round($product['to_default_currency'] * $total_profit)).' '.Currency::DEFAULT_CURRENCY.')</span>' : null);?></th>
    ...
    <?php endif; ?>
        ...
        <ul class="custom-control-group margin-t-2">
            
            <li class="form-check">
                <span>Sales: </span>
                <strong class="margin-l-1"><?=$list_order['sales'];?></strong>
            </li>
            <li class="form-check">
                <span>Agree Buy:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_AGREE_BUY];?></strong>
            </li>
            <li class="form-check">
                <span>Delivery Date:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_DELIVERY_DATE];?></strong>
            </li>
            <li class="form-check">
                <span>Delivering:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_DELIVERING];?></strong>
            </li>

            <li class="form-check">
                <span>Pending Confirm:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_PENDING_CONFIRM];?></strong>
            </li>
            <li class="form-check">
                <span>Calling</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_CALLING];?></strong>
            </li>
            <li class="form-check">
                <span>Busy - Callback:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_BUSY_CALLBACK];?></strong>
            </li>
            <li class="form-check">
                <span>Can Not Call:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_CAN_NOT_CALL];?></strong>
            </li>
            <li class="form-check">
                <span>Wrong Number:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_WRONG_NUMBER];?></strong>
            </li>
            <li class="form-check">
                <span>Refuse Buy:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_REFUSE_BUY];?></strong>
            </li>
            <li class="form-check">
                <span>Duplicate:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_DUPLICATE];?></strong>
            </li>
            <li class="form-check">
                <span>Trash:</span>
                <strong class="margin-l-1"><?=$list_order[Order::STATUS_TRASH];?></strong>
            </li>
