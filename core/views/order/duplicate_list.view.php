...
<div class="box">
    ...
    <div class="box__header"><i class="fad fa-clone"></i><span class="padding-x-2"><?=lang('system', 'order_duplicate');?> - #<?=$order['id'];?></span></div>
    <div class="box__body">
    ...
    <?php if($list_order): ?>
        <div class="margin-b-2">
            <span>
            <?=lang('order', 'txt_count_order_duplicate', [
                'count' => $count,
                'phone' => _echo($order['order_phone'])
            ]);?>
            </span>
        </div>
        ...
        ?>
                    <tr>
                        <td>
                        <?php if($order['call_user_id'] == Auth::$data['id']): ?>
                            <a class="btn btn--primary btn--small" href="<?=RouteMap::get('caller', ['block' => callerController::BLOCK_EDIT, 'action' => $order['id']]);?>">
