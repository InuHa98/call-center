...
<div class="box">
    ...
    <div class="box__header"><i class="fas fa-box-check text-green"></i><span class="padding-x-2"><?=lang('system', 'order_delivered');?></span></div>
    <div class="box__body">
        ...
        <form id="form-filter" method="GET">
            ...
            <div class="filter-bar">
                ...
                <div class="form-control">
                    <select id="filter-payment" class="js-custom-select" name="<?=orderController::FILTER_PAYMENT;?>" data-placeholder="<?=lang('placeholder', 'select_status');?>">
                        <option value="<?=InterFaceRequest::OPTION_ALL;?>"><?=lang('system', 'all_status');?></option>
                        <option <?=($filter_payment == Order::IS_PAYMENT ? 'selected' : null);?> value="<?=Order::IS_PAYMENT;?>"><?=lang('status', 'paid');?></option>
                        <option <?=($filter_payment == Order::IS_NOT_PAYMENT ? 'selected' : null);?> value="<?=Order::IS_NOT_PAYMENT;?>"><?=lang('status', 'unpaid');?></option>
                    </select>
                </div>
            ...
            <table class="table-statistics table-sort">
                <thead>
                    <tr>
                        ...
                        <th></th>
                        <th class="sort-btn"><?=lang('label', 'earning');?></th>
        ...
        <?php 
            ...
            ?>
                    <tr>
                        ...
                        <?php if($order['ship_user_id'] == Auth::$data['id'] && Auth::$data['is_ban_team'] == Team::IS_NOT_BAN): ?>
                            <div class="drop-menu">
                                ...
                                <ul class="drop-menu__content" data-id="<?=$order['id'];?>">
                                    <li role="mark-returned" class="text-danger">
                                        <i class="fas fa-undo-alt"></i> <?=lang('button', 'mark_return');?>
                                    </li>
                                </ul>
                            </div>
                        <?php endif; ?>
                        ...
                        <td class="nowrap">
                            <?php if($amount): ?>
                                <span class="badge rounded-pill bg-success">+<?=Currency::format($amount);?></span>
                            <?php else :?>
                                <span class="badge rounded-pill bg-primary"><?=lang('status', 'paid');?></span>
                            <?php endif; ?>
                        </td>
                        <td class="nowrap">
                        ...
                        <td class="nowrap">
                            <span class="badge btn-outline-success btn-dim"><i class="fas fa-user"></i> <?=_echo($order['order_first_name']);?> <?=_echo($order['order_last_name']);?></span>
                        </td>
            ...
            <?php if($total_amount): ?>
                <tfoot>
                    <tr>
                        ...
                        <th colspan="16" class="align-left"><span class="badge rounded-pill bg-success">+<?=Currency::format($total_amount);?></span></th>
                    </tr>
                </tfoot>
            <?php endif; ?>
            </table>
        ...
        <div class="margin-y-2">
            <span class="drop-menu disabled" role="action-order">
                ...
                <ul class="drop-menu__content">
                    <li role="mark-returned-selected" class="text-danger">
                        <i class="fas fa-undo-alt"></i> <?=lang('button', 'mark_return');?> <span role="multiple_selected_count">(0)</span>
                    </li>
                </ul>
            </span>
        </div>
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        const filter_payment = $('#filter-payment');
        ...
        clear_filter.on('click', function(e) {
            ...
            filter_payment.val(filter_payment.find('option:first').val()).change();
    ...
    <?php if(UserPermission::is_shipper()): ?>
        role_click('mark-returned', function(self) {
			...
			$.dialogShow({
				title: '<?=lang('button', 'mark_return');?> #' + id,
				content: form,
				button: {
        ...
        role_click('mark-returned-selected', function(self) {
