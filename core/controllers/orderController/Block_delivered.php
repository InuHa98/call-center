...
trait Block_delivered {
    private static function block_delivered($team, $action = null) {
        $success = null;
		$error = null;
		$title = lang('system', 'order_delivered');
		$current_route = 'order_delivered';
				...
				case orderController::ACTION_MARK_RETURNED:
					...
					if(!$orders) {
						...
						if($count_update > 0) {
                            ...
                            $orders = Order::list($where);
                            Order::send_notification_returned($orders);
							...
							return redirect_route('order', ['block' => orderController::BLOCK_DELIVERED]);
						} else {
        ...
        if($filter_payment != InterFaceRequest::OPTION_ALL) {
            $where['OR'] = [
                'AND #1' => [
                    'call_user_id' => Auth::$data['id'],
                    'payment_call' => $filter_payment == Order::IS_PAYMENT ? Order::IS_PAYMENT : Order::IS_NOT_PAYMENT
                ],
                'AND #2' => [
                    'ship_user_id' => Auth::$data['id'],
                    'payment_ship' => $filter_payment == Order::IS_PAYMENT ? Order::IS_PAYMENT : Order::IS_NOT_PAYMENT
                ]
            ];
		}
		...
		return View::render('order.delivered', compact(
			'title',
			'current_route',
			'success',
			'error',
