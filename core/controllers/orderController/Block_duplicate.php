...
trait Block_duplicate {
    private static function block_duplicate($team, $id_duplicate = null) {
        $success = null;
		$error = null;
		...
		if($order && $order['duplicate']) {
			$title = lang('system', 'order_duplicate').' - #'.$order['id'];
			$current_route = null;
			$list_status = [];
		} else {
			$title = lang('system', 'order_duplicate');
			$current_route = 'order_duplicate';
			$list_status = [
		...
		if($order && $order['duplicate']) {
			...
			$phones = json_decode($order['order_phone'], true);
			$sql = [];
			$map_phone = [];
			$i = 1;
		...
		return View::render($order && $order['duplicate'] ? 'order.duplicate_list' : 'order.duplicate', compact(
			'title',
			'current_route',
			'success',
			'error',
