...
trait Block_mailer {
	private static function block_mailer($action = null) {
		$title = lang('admin_panel', 'system_mailer');
		$success = null;
		$error = null;
		...
		$smtp_authencation = trim(Request::post(self::INPUT_MAILER_SMTP_AUTHENTICATION, isset($mailer_smtp[Mailer::VAR_AUTHENTICATION]) ? $mailer_smtp[Mailer::VAR_AUTHENTICATION] : false));
		$smtp_username = trim(Request::post(self::INPUT_MAILER_SMTP_USERNAME, isset($mailer_smtp[Mailer::VAR_USERNAME]) ? $mailer_smtp[Mailer::VAR_USERNAME] : null));
		$smtp_password = trim(Request::post(self::INPUT_MAILER_SMTP_PASSWORD, isset($mailer_smtp[Mailer::VAR_PASSWORD]) ? $mailer_smtp[Mailer::VAR_PASSWORD] : null));
		$smtp_host = trim(Request::post(self::INPUT_MAILER_SMTP_HOST, isset($mailer_smtp[Mailer::VAR_HOST]) ? $mailer_smtp[Mailer::VAR_HOST] : null));
		$smtp_secure = trim(Request::post(self::INPUT_MAILER_SMTP_SECURE, isset($mailer_smtp[Mailer::VAR_SECURE]) ? $mailer_smtp[Mailer::VAR_SECURE] : null));
		$smtp_port = trim(Request::post(self::INPUT_MAILER_SMTP_PORT, isset($mailer_smtp[Mailer::VAR_PORT]) ? $mailer_smtp[Mailer::VAR_PORT] : null));

		$api_server = trim(Request::post(self::INPUT_MAILER_API_SERVER, isset($mailer_api[Mailer::VAR_SERVER]) ? $mailer_api[Mailer::VAR_SERVER] : null));
		$api_key = trim(Request::post(self::INPUT_MAILER_API_KEY, isset($mailer_api[Mailer::VAR_KEY]) ? $mailer_api[Mailer::VAR_KEY] : null));
		$api_secret = trim(Request::post(self::INPUT_MAILER_API_SECRET, isset($mailer_api[Mailer::VAR_SECRET]) ? $mailer_api[Mailer::VAR_SECRET] : null));
		...
		if(Security::validate() == true)
        {
			if($mailer_from != '' && !Auth::check_type_email($mailer_from)) {
				$error = lang('system_mailer', 'error_email');
			} else {
