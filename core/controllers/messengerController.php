...
class messengerController {
	...
	public function new($user_id = null)
	{
		...
		if(isset($find_user))
		{
			if(!$find_user)
			{
				$data_view['error'] = lang('errors', 'user_not_found');
			}
			else
			{
		...
		return redirect_route('messenger');
	}
	...
	public function inbox($id = null, $is_spam = false)
	{
		...
		if(Security::validate() == true)
        {
            ...
            switch($form_action)
            {
				case self::ACTION_MAKE_SPAM:
					...
					{
						$success = lang('messenger', 'success_move_spam');
					}
					else
					{
				...
				case self::ACTION_MAKE_INBOX:
					...
					{
						$success = lang('messenger', 'success_move_inbox');
					}
					else
					{
						$error = lang('system', 'default_error');
		...
		return View::render('messenger.inbox', compact('title', 'error', 'success', 'is_spam', 'type', 'keyword', 'inbox_data', 'pagination'));
	}
	...
	public function chat($id = null)
	{
		...
		if($page)
		{
			...
			if($count > 0)
			{
				...
				if(!$messages)
				{
					echo json_api(404, lang('messenger', 'item_not_found'), $data_messages);
					exit;
				}
			...
			echo json_api(200, count($data_messages).' '.lang('messenger', 'message'), $data_messages);
			exit;
		}


		$title = lang('messenger', 'title').' - '.$user_to['username'];
        ...
        if(Security::validate() == true)
        {
            ...
            switch($form_action)
            {
                ...
                case self::ACTION_SEND_MESSAGE:
					$message = trim(Request::post(self::INPUT_MESSAGE, null));
		...
		return View::render('messenger.chat', compact('title', 'error', 'success', 'user_to', 'is_spam', 'count'));
	}
