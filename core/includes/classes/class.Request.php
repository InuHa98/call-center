...
class Request {
	...
	public static function files($key = null, $default = '')
	{
		if($key == '')
		{
			return $_FILES;
	...
	public static function build_input_get($except = []) {
		if(!is_array($except)) {
			$except = [$except];
		}
		if(count($_GET) > 0) {
			foreach($_GET as $k => $v) {
				if(!in_array($k , $except)) {
					if(is_array($v)) {
						foreach($v as $v_arr) {
							echo '<input type="hidden" name="'._echo($k).'[]" value="'._echo($v_arr).'" />';
						}
					} else {
						echo '<input type="hidden" name="'._echo($k).'" value="'._echo($v).'" />';
					}
				}
			}
		}
