...
abstract class Model {
	...
	protected static $tmp_select = null;
	protected static $tmp_join = null;
	...
	{
		$where = $where ? $where : static::$where;
		if(!is_array($where))
		{
			$where = [];
		}
	...
	public static function count($where = [], $select = null)
	{
		...
		return App::$database->count($select ? self::statement($select) : static::$table, $where);	
	}

	public static function has($where = [], $select = null)
	{
		self::debug_mode();
		return App::$database->has($select ? self::statement($select) : static::$table, static::build_where($where));	
	}
