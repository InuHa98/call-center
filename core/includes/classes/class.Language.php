...
class Language
{
    ...
    private const ERROR_NOT_FOUND = '{{%s: %s}}';
    public const COOKIE_NAME = '_lang';
    ...
    public static function init($config = null)
    {
        ...
        self::$browser = substr(Request::server('HTTP_ACCEPT_LANGUAGE', null), 0, 2);
    ...
    public static function load($file_name = null)
    {
        ...
        $file_name = array_map(function($name) {
            return str_replace(self::FILE_EXT, '', $name).self::FILE_EXT;
        }, $file_name);
    ...
    public static function get($section = null, $key = null, $data = [])
    {
        ...
        if($section)
        {
            $result =  isset(self::$data[$section][$key]) ? self::$data[$section][$key] : sprintf(self::ERROR_NOT_FOUND, $section, $key);
        }
        else
        {
            $result = isset(self::$data[$key]) && !is_array(self::$data[$key]) ? self::$data[$key] : sprintf(self::ERROR_NOT_FOUND, $section, $key);
        }
